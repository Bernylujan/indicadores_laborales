...
function BaseTooltip({ label, rows, rawLabel = false }) {
  return (
    <div style={{
      ...
      fontSize: 10, maxWidth: 220, pointerEvents: "none",
    }}>
      <div style={{ fontWeight: 700, color: MX.vino, marginBottom: 5, fontSize: 10 }}>
      ...
      {rows.map((r, i) => (
        <div key={i} style={{
          ...
          paddingTop: r.sep ? 5 : 0, marginTop: r.sep ? 3 : 0,
        }}>
...
function KpiCard({ label, valor, fmt, nacStr, delta, dDir, nota, color, isMobile }) {
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} style={{
      ...
      <div style={{ fontSize: isMobile ? 8 : 9, fontWeight: 700, letterSpacing: isMobile ? 1 : 2, color: MX.grayMid, textTransform: "uppercase", marginBottom: isMobile ? 5 : 8 }}>{label}</div>
      <div style={{ fontSize: isMobile ? 22 : 32, fontWeight: 700, color, lineHeight: 1, marginBottom: isMobile ? 5 : 8 }}><AnimNum target={valor} fmt={fmt} /></div>
      {nacStr && <div style={{ fontSize: isMobile ? 9 : 10, color: MX.grayMid, marginBottom: 3 }}>Nacional: <b style={{ color: MX.grayDark }}>{nacStr}</b></div>}
      {delta  && <div style={{ fontSize: isMobile ? 9 : 10, fontWeight: 700, color: dDir === "pos" ? MX.green : "#C0392B", marginBottom: 3 }}>{dDir === "pos" ? "▲" : "▼"} {delta} vs año anterior</div>}
    ...
    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
      {options.map(o => (
        <button key={o.id} onClick={() => onChange(o.id)} style={{
          fontFamily: FONT, fontSize: isMobile ? 10 : 11, fontWeight: 600, cursor: "pointer",
          padding: isMobile ? "4px 12px" : "5px 16px", borderRadius: 20, transition: "all .15s",
...
function Card({ title, children, style, isMobile }) {
  return (
    <div style={{ background: MX.white, border: `1px solid ${MX.border}`, borderRadius: 10, padding: isMobile ? "14px 12px" : "18px 20px", boxShadow: "0 2px 8px rgba(0,0,0,0.05)", ...style }}>
      {title && <div style={{ fontFamily: FONT, fontSize: isMobile ? 10 : 11, fontWeight: 700, color: MX.vino, marginBottom: 12, lineHeight: 1.4 }}>{title}</div>}
      {children}
    </div>
  );
}
...
function GrafPEAOcupados({ isMobile }) {
  ...
  const TooltipPEA = ({ active, payload, label }) => {
    ...
    return (
      <BaseTooltip label={label} rows={[
        { name: "PEA",        value: fmtN(row.pea),       color: MX.vino    },
        { name: "Ocupados",   value: fmtN(row.ocup),      color: MX.rosa    },
        { name: "Sin empleo", value: fmtN(row.desoc_abs), color: MX.vinoMid, sep: true },
      ]} />
    );
  };
    ...
    <Card title="PEA vs Población Ocupada — Michoacán" isMobile={isMobile} style={{ width: "100%" }}>
      <ResponsiveContainer width="100%" height={isMobile ? 200 : 230}>
        <ComposedChart data={data} margin={{ left: isMobile ? 4 : 14, right: isMobile ? 4 : 14, top: 10, bottom: 0 }}>
          ...
          <YAxis tick={axTick({ fontSize: isMobile ? 7 : 8 })} axisLine={false} tickLine={false} tickFormatter={fmtM} domain={[1700000, 2450000]} width={isMobile ? 36 : 46} />
...
function GrafTasas({ isMobile }) {
  ...
  const TooltipTasas = ({ active, payload, label }) => {
    ...
    const seen = new Set();
    const rows = payload
      .filter(p => { const dup = seen.has(p.dataKey); seen.add(p.dataKey); return !dup; })
      .map(p => ({ name: p.name, value: `${Number(p.value).toFixed(2)}%`, color: p.color }));
    ...
    <Card title="Tasa de Participación y Tasa de Desocupación (%)" isMobile={isMobile} style={{ width: "100%" }}>
      <ResponsiveContainer width="100%" height={isMobile ? 200 : 230}>
        <ComposedChart data={data} margin={{ left: isMobile ? 4 : 14, right: isMobile ? 28 : 34, top: 5, bottom: 0 }}>
          ...
          <CartesianGrid strokeDasharray="3 3" stroke="#F0E8EC" />
          <XAxis dataKey="p" tick={axTick({ fontSize: isMobile ? 7 : 8 })} axisLine={false} tickLine={false} tickFormatter={xTickYear} interval={0} />
          <YAxis yAxisId="L" tick={axTick({ fontSize: isMobile ? 7 : 8 })} axisLine={false} tickLine={false} tickFormatter={v => `${v.toFixed(0)}%`} domain={[50, 65]} width={isMobile ? 26 : 36} />
...
function GrafSubsectores({ isMobile }) {
  ...
  const selectStyle = {
    fontFamily: FONT, fontSize: isMobile ? 10 : 11, fontWeight: 600, cursor: "pointer",
    padding: isMobile ? "4px 10px" : "5px 12px", borderRadius: 20, border: `1.5px solid ${MX.vino}`,
    color: MX.vino, background: MX.crema, outline: "none", appearance: "none",
    WebkitAppearance: "none", paddingRight: isMobile ? 22 : 28,
  };

  const TooltipSub = ({ active, payload, label }) => {
    if (!active || !payload?.length) return null;
    return <BaseTooltip label={label} rows={[{ name: "Ocupados", value: `${fmtN(payload[0]?.value)} personas`, color: MX.vino }]} rawLabel />;
  };

  const yAxisW   = isMobile ? 108 : 205;
  const chartH   = isMobile ? 340 : 360;
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 14 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          <div style={{ position: "relative" }}>
            ...
            <span style={{ position: "absolute", right: isMobile ? 7 : 10, top: "50%", transform: "translateY(-50%)", pointerEvents: "none", color: MX.vino, fontSize: 10 }}>▾</span>
          </div>
          <div style={{ position: "relative" }}>
          ...
          {!periodos.includes(perKey) && (
            <span style={{ fontFamily: FONT, fontSize: 9, color: MX.rosa }}>No disponible — mostrando {periodoValido}</span>
          )}
        </div>
      </div>
      <ResponsiveContainer width="100%" height={chartH}>
        <BarChart data={barData} layout="vertical" margin={{ left: 4, right: isMobile ? 48 : 80, top: 5, bottom: 5 }}>
          ...
          <YAxis type="category" dataKey="label" tick={axTick({ fontSize: tickSize })} axisLine={false} tickLine={false} width={yAxisW} />
          ...
          <Bar dataKey="val" name="Ocupados" radius={[0, 4, 4, 0]}>
            ...
            <LabelList dataKey="val" position="right"
              style={{ fontFamily: FONT, fontSize: tickSize, fill: MX.grayDark, fontWeight: 600 }}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: isMobile ? 18 : 24 }}>
      ...
      <Section title="Fuerza Laboral" sub="ENOE Trimestral 2016–2025"
        ...
        active={secFL} onChange={setSecFL} isMobile={isMobile}>
      ...
      <Section title="Dinámica del Mercado Laboral" sub="ENOE Trimestral 2016–2025"
        ...
        active={secDin} onChange={setSecDin} isMobile={isMobile}>
...
function TabIMSS({ isMobile }) {
  ...
  const makeTooltip = (labelMap) => ({ active, payload, label }) => {
    ...
    const seen = new Set();
    const rows = payload
      .filter(p => { if (!p.dataKey || seen.has(p.dataKey)) return false; seen.add(p.dataKey); return true; })
      .map(p => ({ name: labelMap?.[p.dataKey] || p.name, value: fmtN(p.value), color: p.stroke || p.fill || MX.grayDark }));
    return <BaseTooltip label={label} rows={rows} />;
  };
  ...
  const TooltipBar = ({ active, payload, label }) => {
    if (!active || !payload?.length) return null;
    return <BaseTooltip label={label} rows={[{ name: "Puestos de trabajo", value: fmtN(payload[0]?.value), color: MX.vino }]} rawLabel />;
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: isMobile ? 18 : 24 }}>
      ...
      <Section title="Puestos de Trabajo" sub="IMSS Mensual 2016–2026"
        ...
        active={g1} onChange={setG1} isMobile={isMobile}>

        {g1 === "anual" && (
          <Card title="Total de Puestos de Trabajo Asegurados (enero de cada año)" isMobile={isMobile} style={{ width: "100%" }}>
            <ResponsiveContainer width="100%" height={isMobile ? 230 : 280}>
              <BarChart data={barData} margin={{ left: isMobile ? 4 : 10, right: isMobile ? 4 : 10, top: 28, bottom: 0 }}>
                <XAxis dataKey="anio" tick={axTick({ fontSize: isMobile ? 8 : 9 })} axisLine={false} tickLine={false} interval={isMobile ? 1 : 0} />
        ...
        {g1 === "comp" && (
          <Card title="Permanentes vs Eventuales — serie mensual" isMobile={isMobile} style={{ width: "100%" }}>
            <ResponsiveContainer width="100%" height={isMobile ? 200 : 240}>
              <ComposedChart data={compData} margin={{ left: isMobile ? 4 : 14, right: isMobile ? 4 : 20, top: 5, bottom: 0 }}>
                <defs>
                ...
                <CartesianGrid strokeDasharray="3 3" stroke="#F0E8EC" />
                <XAxis dataKey="p" tick={axTick({ fontSize: isMobile ? 7 : 8 })} axisLine={false} tickLine={false} tickFormatter={xTickIMSS} interval={0} />
                <YAxis tick={axTick({ fontSize: isMobile ? 7 : 8 })} axisLine={false} tickLine={false}
                  tickFormatter={v => `${(v/1000).toFixed(0)}k`} domain={[0, 470000]} width={isMobile ? 30 : 40} />
      ...
      <Section title="Desglose por zona" sub="Permanentes y Eventuales"
        ...
        active={g2} onChange={setG2} isMobile={isMobile}>

        {g2 === "urb" && (
          <Card title="Zona Urbana — Permanentes y Eventuales" isMobile={isMobile} style={{ width: "100%" }}>
            <ResponsiveContainer width="100%" height={isMobile ? 200 : 240}>
              <ComposedChart data={dataUrb} margin={{ left: isMobile ? 4 : 14, right: isMobile ? 4 : 20, top: 5, bottom: 0 }}>
                <defs>
                ...
                <XAxis dataKey="p" tick={axTick({ fontSize: isMobile ? 7 : 8 })} axisLine={false} tickLine={false} tickFormatter={xTickIMSS} interval={0} />
                <YAxis tick={axTick({ fontSize: isMobile ? 7 : 8 })} axisLine={false} tickLine={false}
                  tickFormatter={v => `${(v/1000).toFixed(0)}k`} domain={[0, 430000]} width={isMobile ? 30 : 40} />
        ...
        {g2 === "campo" && (
          <Card title="Zona Campo — Permanentes y Eventuales" isMobile={isMobile} style={{ width: "100%" }}>
            <ResponsiveContainer width="100%" height={isMobile ? 200 : 240}>
              <ComposedChart data={dataEvCampo} margin={{ left: isMobile ? 4 : 14, right: isMobile ? 4 : 20, top: 5, bottom: 0 }}>
                <defs>
                ...
                <XAxis dataKey="p" tick={axTick({ fontSize: isMobile ? 7 : 8 })} axisLine={false} tickLine={false} tickFormatter={xTickIMSS} interval={0} />
                <YAxis tick={axTick({ fontSize: isMobile ? 7 : 8 })} axisLine={false} tickLine={false}
                  tickFormatter={v => `${(v/1000).toFixed(0)}k`} domain={[0, 40000]} width={isMobile ? 30 : 40} />
...
function TabSectores({ isMobile }) {
  ...
  const TooltipPie = ({ active, payload }) => {
    if (!active || !payload?.length) return null;
    const d = payload[0];
    return <BaseTooltip label={d.name} rows={[{ name: "Participación", value: `${d.value}%`, color: d.payload?.c }]} rawLabel />;
  };

  const TooltipSub2 = ({ active, payload, label }) => {
    if (!active || !payload?.length) return null;
    return <BaseTooltip label={label} rows={[{ name: bar === "enoe" ? "Trabajadores" : "Asegurados", value: fmtN(payload[0]?.value), color: MX.vino }]} rawLabel />;
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: isMobile ? 18 : 24 }}>
      ...
      <Section title="Distribución por sector" sub="Comparativo ENOE · IMSS"
        ...
        active={pie} onChange={setPie} isMobile={isMobile}>
        <Card title={pie==="enoe"
          ? `ENOE — Trabajadores subordinados y remunerados por sector (${perValid})`
          : "IMSS — Asegurados por gran sector (Ene 26)"} isMobile={isMobile} style={{ width: "100%" }}>
          {/* En mobile: pie arriba, leyenda debajo centrada */}
          <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: isMobile ? 10 : 32, alignItems: "center" }}>
            <div style={{ width: "100%", maxWidth: isMobile ? "100%" : 310 }}>
              <ResponsiveContainer width="100%" height={isMobile ? 200 : 250}>
                <PieChart>
                  <Pie data={pieData} dataKey="v" nameKey="s" cx="50%" cy="50%" outerRadius={isMobile ? 75 : 100} paddingAngle={2} label={<PieLabel />} labelLine={false}>
            ...
            <div style={{ display: "flex", flexDirection: isMobile ? "row" : "column", flexWrap: "wrap", gap: isMobile ? "8px 18px" : 10, justifyContent: isMobile ? "center" : "flex-start" }}>
              {pieData.map((d, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ width: isMobile ? 10 : 12, height: isMobile ? 10 : 12, borderRadius: "50%", background: d.c, flexShrink: 0 }} />
                  <span style={{ fontFamily: FONT, fontSize: isMobile ? 11 : 11, color: MX.grayDark, fontWeight: 600 }}>{d.s}</span>
                  <span style={{ fontFamily: FONT, fontSize: isMobile ? 11 : 11, color: MX.grayMid }}>{d.v}%</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <Section title="Distribución por Sector de Actividad Económica"
        options={[{id:"enoe",label:"Vista ENOE"},{id:"imss",label:"Vista IMSS"}]}
        active={bar} onChange={setBar} isMobile={isMobile}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 14 }}>
            ...
            {bar === "enoe" && (
              <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                ...
                {!periodos.includes(perKey) && (
                  <span style={{ fontFamily: FONT, fontSize: 9, color: MX.rosa }}>No disponible — mostrando {perValid}</span>
                )}
              </div>
            )}
          </div>
          <ResponsiveContainer width="100%" height={isMobile ? 380 : 360}>
            <BarChart data={barData} layout="vertical" margin={{ left: 4, right: isMobile ? 44 : 80, top: 5, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#F0E8EC" horizontal={false} />
              <XAxis type="number" tick={axTick({ fontSize: isMobile ? 7 : 8 })} axisLine={false} tickLine={false} tickFormatter={v => `${(v/1000).toFixed(0)}k`} />
              <YAxis type="category" dataKey="label" tick={axTick({ fontSize: isMobile ? 7.5 : 9 })} axisLine={false} tickLine={false} width={isMobile ? 110 : 205} />
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: MX.grayLt, fontFamily: FONT, color: MX.grayDark }}>
      {/* HEADER */}
      <div style={{
        background: MX.white, borderBottom: `3px solid ${MX.vino}`,
        padding: isMobile ? "12px 16px" : "14px 36px",
        display: "flex", justifyContent: "space-between",
        alignItems: isMobile ? "flex-start" : "center", gap: isMobile ? 8 : 0,
      }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: isMobile ? 15 : 19, fontWeight: 700, color: MX.vinoDark, lineHeight: 1.2 }}>Dashboard de Indicadores Laborales</div>
        </div>
        <div style={{ textAlign: "right", flexShrink: 0 }}>
          <div style={{ fontSize: 8, color: MX.grayMid, marginBottom: 3 }}>Último período</div>
          <div style={{ fontSize: isMobile ? 10 : 11, fontWeight: 700, color: MX.vino, background: MX.crema, border: `1px solid ${MX.border}`, borderRadius: 6, padding: isMobile ? "4px 10px" : "4px 14px", textAlign: "center" }}>
            IV Trim. 2025
          </div>
        </div>
      </div>
      ...
      <div style={{
        background: MX.white, borderBottom: `1px solid ${MX.border}`,
        overflowX: "auto", WebkitOverflowScrolling: "touch",
        scrollbarWidth: "none", msOverflowStyle: "none",
        display: "flex", paddingLeft: isMobile ? 0 : 36,
      }}>
        {NAV.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            fontFamily: FONT, background: "transparent", border: "none",
            borderBottom: tab === t.id ? `3px solid ${MX.vino}` : "3px solid transparent",
            padding: isMobile ? "10px 16px" : "12px 22px", cursor: "pointer", transition: "all .15s",
            display: "flex", flexDirection: "column", alignItems: "center", gap: 2, flexShrink: 0,
          }}>
            <span style={{ fontSize: isMobile ? 11 : 12, fontWeight: tab === t.id ? 700 : 400, color: tab === t.id ? MX.vino : MX.grayMid, whiteSpace: "nowrap" }}>{t.label}</span>
            <span style={{ fontSize: 9, color: tab === t.id ? MX.rosa : "#ccc", whiteSpace: "nowrap" }}>{t.sub}</span>
          </button>
        ))}
      </div>
